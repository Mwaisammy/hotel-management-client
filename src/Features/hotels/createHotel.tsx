...
const CreateHotel = ({ hotel }: CreateHotelProps) => {
  const [createRoom, { isLoading }] = hotelsAPI.useCreateHotelMutation();
  ...
  const onSubmit: SubmitHandler<CreateRoomInputs> = async (data) => {
    try {
      ...
      reset();
      (
        document.getElementById("create_hotel_modal") as HTMLDialogElement
      )?.close();
    } catch (error) {
